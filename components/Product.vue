<template>
  <div class="bg-white">
    ...
    <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-10 lg:max-w-7xl lg:px-8">
      ...
      <Alert v-if="isUnauthorized"></Alert>
      <div class="mt-6 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
        <div v-for="product in products" :key="product.id" class="group relative">
          <div class="aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-md bg-gray-200 lg:aspect-none group-hover:opacity-75 lg:h-80">
            <img :src="`http://wallserver.dyndns.info:5000${product.image}`" class="h-full w-full object-cover object-center lg:h-full lg:w-full" />
          </div>
...
import useFetchProducts from '~/composables/FetchStore';
const { products, isUnauthorized, fetchProducts } = useFetchProducts();
